...
講師から共有された GitHub の URL からクローンしてください。

```bash
git clone https://github.com/rafau1784-blip/aizemi-workspace-ui-kit.git
cd aizemi-workspace-ui-kit
```

Cursor でこのフォルダを開き、以降の手順に進みます。

### Cursor のチャットでクローンする場合

チャットに次を貼り付けて送信してください。

```text
https://github.com/rafau1784-blip/aizemi-workspace-ui-kit をクローンして。
保存場所を聞かれたら、わからなければ「おまかせ」と答えてください。
完了したら、そのフォルダを Cursor の新しいウィンドウで開いてください。
```
...
課題の取り組み方の詳細は**学習ポータル**（【3ヶ月目】ひな形を自分の画面に仕上げる）を参照してください。

## 提出する

提出物・期限・中間発表のフォーマットは**学習ポータル**を見てください。

## 困ったとき

1. **AI に聞く** — エラーメッセージやスクリーンショットをチャットに貼る
2. **AI-ZEMI Slack の質問チャンネル** — 「AI にこう聞いたが解決しなかった」と経緯を添えて相談
3. **問い合わせフォーム** — 試したことをすべて書いて運営に連絡

- Slack の DM・メンションでの個別質問には対応していません
- 運営への連絡は**必ずフォーム**から

---

Mac / Windows どちらでも使えます。AI-ZEMI 運営が動作確認済みです。
